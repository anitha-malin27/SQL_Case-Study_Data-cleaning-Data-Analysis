SELECT * FROM painting.artist; 
SELECT * FROM painting.canvas_size;
SELECT * FROM painting.image_link;
SELECT * FROM painting.museum;
SELECT * FROM painting.museum_hours;
SELECT * FROM painting.product_size;
SELECT * FROM painting.subject;
SELECT * FROM painting.work;
...
#5) Which canva size costs the most?#
	...
	join canvas_size cs on cs.size_idtext=ps.size_id
	where ps.rnk=1;					 
...
#14) Display the 3 least popular canva sizes
	...
	from (
		...
		join canvas_size cs on cs.size_id 
...
#18) Display the country and the city with most no of museums. Output 2 seperate columns to mention the city and country. If there are multiple value, seperate them with comma.
	with cte_country as 
		...
		cte_city as
			...
			from museum
			group by city)
	select group_concat(distinct country), group_concat(city)
	from cte_country 
	cross join cte_city 
